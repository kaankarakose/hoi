...
def run_inference(template_dir, rgb_path, num_max_dets, conf_threshold, stability_score_thresh):
    ...
    frames_path = [ os.path.join(rgb_path, f) for f in os.listdir(rgb_path) if f.endswith('.jpg') ]

    for i, path in enumerate(frames_path[30:]):
        rgb = Image.open(path).convert("RGB")
        # Calculate target size that's divisible by 14
        w, h = rgb.size
        target_w = ((w + 7) // 14) * 14
        target_h = ((h + 7) // 14) * 14
        ...
        # # Standardize mask sizes
        # masks = getattr(detections, 'masks')
        # if len(masks) > 0:
        #     # Get original size
        #     orig_h, orig_w = masks[0].shape
        #     # Round to nearest multiple of 14
        #     target_h = ((orig_h + 7) // 14) * 14  # Round up to nearest multiple of 14
        #     target_w = ((orig_w + 7) // 14) * 14
        ...
        #         standardized_masks.append(torch.from_numpy(mask_np).to(masks[0].device))
            
        #     detections.masks = torch.stack(standardized_masks)
        ...
        output_dir = '/nas/project_data/B1_Behavior/rush/ados-objects/object_pose_mega/rtdt/cnos/cnos/output_amf1'
        # save_path = f"{output_dir}/cnos_results/detection"
        # print(f'Saving results to: {save_path}')
        # detections.save_to_file(0, 0, 0, save_path, "custom", return_results=False)
        
        # First visualize with original detections
        vis_img = visualize(rgb, detections)
        vis_img.save(f"{output_dir}/cnos_results/vis_{i:06d}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_max_dets", type=int, default=5, help="Maximum number of detections")
    parser.add_argument("--conf_threshold", type=float, default=0.5, help="Confidence threshold")
    parser.add_argument("--stability_score_thresh", type=float, default=0.7, help="Stability score threshold")
    args = parser.parse_args()


    output_folder = '/nas/project_data/B1_Behavior/rush/ados-objects/object_pose_mega/rtdt/cnos/cnos/output_amf1'
    os.makedirs(f"{output_folder}/cnos_results", exist_ok=True)
    run_inference(args.template_dir, args.rgb_path, num_max_dets=args.num_max_dets, conf_threshold=args.conf_threshold, stability_score_thresh=args.stability_score_thresh)